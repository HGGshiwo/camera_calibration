...
class CameraCalibrator:
    ...
    def draw_corners(self, frame):
        ...
        if ret:
            ...
            # 添加检测提示
            cv2.putText(frame, "corner detected!", (10, frame.shape[0] - 20),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        else:
            cv2.putText(frame, "no corner", (10, frame.shape[0] - 20),
    ...
    def auto_calibrate(self, camera, progress_callback: Callable, max_images: int = 30, min_images: int = 15):
        """
        自动标定
        Args:
            camera: 摄像头对象
        ...
        while not self.stop_calibration and collected < max_images:
            ...
            if ret and (current_time - last_capture_time) > capture_interval:
                ...
                self.current_instruction = f"已采集 {collected}/{max_images} 张图片。{angle_hint}"
                ...
                self.current_instruction = f"保持姿势... {remaining_time:.1f}秒后采集下一张"
            else:
                # 未检测到棋盘格
            ...
            if collected >= min_images:
                self.current_instruction += f" ({collected}/{max_images})，按停止键可提前完成"
        ...
        if final_images < min_images:
            progress_callback(0, f"采集的图片数量不足，需要至少 {min_images} 张，当前只有 {final_images} 张")
            raise ValueError(f"采集的图片数量不足，需要至少 {min_images} 张，当前只有 {final_images} 张")
        
        print(f"图片采集完成，共采集 {final_images} 张")
        progress_callback(100, f"图片采集完成，共 {final_images} 张，正在计算标定参数...")
